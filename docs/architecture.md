...
| `samples/` | Fixtures for every schema version and the dependency-free tests. `samples/README.md` is the inventory. |
...
The heart is a home page and tracking is date-first; `docs/design-rationale.md` holds the reasoning
and should be read before changing any of this.

`showRoute(route)` knows six routes and toggles four views, one visible at a time.
**It is the only thing that touches their `hidden` flags** — never toggle a surface from a handler.
...
- **Header** (sticky, two rows) — a back arrow and a tappable context line on row 1; a worded Overview/Log toggle and `n/m done` on row 2, with either the progress bar (Overview) or the pip strip (Log) beneath. Nothing here is an input. **The context line reads differently per view**: the block name and `Week n of m` in Overview, where the date view states the date itself directly below, and `Week 3 · Thu 3 Sep` over the day's theme in Log, where there is no date head. Saying the date twice on one screen is duplication. Tapping it opens the claim picker. The progress bar has **two fill layers** (`#pbarDone`, and the dimmer `#pbarPart` behind it) so a session with four exercises half-logged doesn't read as 0% through, and the pips carry **three** states, not two (`data-done`, `data-part`).
- **`<main>` in Overview — the date view.** In order: `dateHead()` (the date, what is on it, its state, one primary action, and the quiet claim action), any banners, the check-in, the exercise cards, then `dateFoot()` (Finish session, or the seal/export status). **The check-in never gates Start**, which is why the primary action sits above it.
  - **Overview always renders a status line** (see `statusLine()`), plus a right-aligned `○ / ◐ / ✓` badge. There must always be one: an absent line cannot be told apart from a status that failed to render, and it would leave a typed-but-uncommitted set invisible everywhere outside the editor.
- **`<main>` in Log — the focus logger.** Exactly one exercise. Order is load-bearing: name → prescription → the `Capture` chips → **the instrument rail** (rest clock and "last time") → **the set editor** → coach notes / progression rule / Summary & notes. The reference material must stay *below* the editor: three collapsibles above the fields push the primary action off a 320×568 screen.
...
The inline workout script is organised in banner-delimited sections. **The order below is the
order in the file** — keep additions in the matching section, and if you add one, add it here.

1. **helpers** — `$(sel)`, `el(tag, props, kids)`, `toast()`, `slug()`, `showRoute()` and the `open*Page()` helpers, `bootSurface()`, cached-programme entry and explicit sample entry.
2. **local calendar days** — `isoOf()`, `todayISO()`, `parseISO()`, `addDays()`, `weekdayOf()`, `mondayOf()`, `daysBetween()`. **Every date in the app is a local calendar day, never a UTC instant**: `new Date("2026-01-01")` parses as midnight UTC and lands on 31 December west of Greenwich, which would shift a whole block by a day. Nothing may reach for `Date.parse`, and `daysBetween()` rounds because a DST boundary makes the raw difference 23 or 25 hours.
3. **settings** — `FIELD_DEFS`, the two scopes (`ACCOUNT_KEYS` / `DEVICE_KEYS`), `loadSettings()` (which also migrates a pre-split install), `saveSettings()` (device half), `setSetting(key, value)` — **the only way to write a setting** — `adoptAccountSettings()`, `painLbl()`/`painLblShort()`, `renderFields(host)`.
4. **theme** — `PALETTES`, `MODES`, `themeId()`, `applyTheme()`, `renderAppearance(host)`. Two palettes (Amber / Mint) × light/dark = the four `html[data-theme]` blocks at the top of the `<style>`. A `matchMedia` listener re-applies on device light/dark change, but only while mode is `auto`. A ~6-line script in `<head>` duplicates `themeId()` so the correct palette is painted before the body renders — **if you change the resolution rule, change both.**
5. **what a session says about itself** — `checkinSummary()`/`checkinFilled()`, which drive the check-in's own one-line status.
6. **moving around the block** — `selectDay()`, `selectWeek()`, `selectDate()`. Each is either a **claim** (a write) or a **date change** (a re-resolve), never an independent axis that could drift out of step.
7. **exercise categories** — `CATS`, `CAT_ALIASES`, `CAT_RULES`, `catOf(ex)`. Rail colour + tag; see `docs/data-contracts.md` for the fallback ladder.
8. **program loading** — `isV2()` and `dayExercises()` (the schema fork; see quirks), `dayLabels()` (**the only correct way to read `meta.days`** — a structured day entry is an object, and letting one reach `STATE.day` would break the session key, the day filter and the header at once), `athleteId()`, `loadProgram(obj)`. `boot()` loads settings then `bootSurface()`; `openCachedWorkout()` restores `tp_program_v1`, `openSampleWorkout()` fetches the pre-cached sample without replacing it.
9. **what the "reps" field actually measures** — `metricOf(ex)`. See the quirk below.
10. **what the programme says to capture** — `parseHints(ex)` (the `Capture` chips), `painAsked(ex)`/`programPainSite()`. Read-only readers of the prescription, all pure.
11. **where the athlete is in the block** — `suggestedWeek()` and the position helpers. Position is always derived, never stored.
12. **the derived schedule** — `WEEKDAYS`/`parseWeekday()`, `dayEntries()`, `programKeyOf()`, `anchorFor()`/`saveAnchor()`/`setAnchorMonday()`, `scheduleFor(program)`, `refreshSchedule()` and the live `SCHEDULE`, then the two directions: `dateFor(week, dayLabel)` and `scheduleForDate(date)`.
13. **what a date is** — `dateStateOf(iso)` (the five states), `nextScheduled()`, `sessionStored()`, `openState()`, `adoptDate()`, `openDate()`, `applyClaim(week, day)`, `startSession()`.
14. **sealing** — `sealSession()`, `unsealSession()`, `markExported()`. Local, works with the radio off, and an edit never un-seals. Also `prescribedSets(ex)`/`targetSets(ex, e)`.
15. **session persistence** — `sessionKey()` (empty when no day is claimed, so a stray autosave cannot file a `::null` record), `getSession()`, `saveSession()`. Device storage always completes before `TPSessions.stage()` snapshots personal work for asynchronous sync.
16. **deriving the flat summary from `sets[]`** — `deriveSummary()`/`deriveReps()`/`withUnit()`/`pickMode()`/`pickMax()`/`applyDerivedSummary()`. Must fail safe on empty or partial data; see the quirk below.
17. **migrating a stored entry** — `normalizeEntry()`. Runs on every read, so it is idempotent by construction; see below.
18. **session lifecycle state** — `normalizeSessionState()`/`sealed()`/`editedSinceExport()`/`nextStamp()`, and the per-key contract for `status`/`sealedAt`/`exportedAt`/`editedAt`/`lastSetAt`.
19. **the session index** — `sessionIndex()`, `invalidateSessionIndex()`, `sessionsOn(date)`, `lastLoggedFor(ex, date)`, `normName()`. A **cache** over the sibling `tp_sess_v1::*` keys, built once and dropped by every write. `saveSession()` runs on every keystroke, so **never call the builder from a render path**.
20. **set mutation** — `blankEntry()`, `blankDraft()`/`seedDraft()`/`draftEmpty()`/`draftDirty()`, `entryState()`, `renumber()`, `commitSet()`, `deleteSet()`, and the two that make a typed set unloseable: `commitDraftIfDirty()`, `flushDraft()`. Every commit path takes the session so `stampLastSet()` can write `lastSetAt` — the rest clock's only state.
21. **rendering** — `renderAll()` = `renderHeader()` + `renderMain()`; `dayParts()`, `dayTheme()`, the `fmt*`/`relDay()` date formatters, `field(label, node, unit, labelClass)`, `updateProgress()`.
22. **view mode** — `viewMode()`, `setView()`, `syncView()` (toggle state, footer/header visibility, and capture/restore of the runtime-only Overview scroll position), `firstUndone()`, `resetFocus()`, `clampFocus()`, `goFocus(i)`, `stepFocus(d)`, `renderNav()` (rebuilds the pips) and `paintNav()` (repaints their state without touching structure).
23. **the session check-in** — `sessionCard()`, rendered into `#checkinHost` by `renderCheckin()`, which rebuilds *only* that block because the exercise list below holds a scroll position a full rebuild would throw away.
24. **the date view** — `dateHead()`, `dateFoot()`, `sessionProgressLine()`, `sealLine()`, `openClaimPicker()` (week and day together, the current selection preselected), `confirmFinish()`, `confirmReopen()`.
25. **the hub** — `renderHome()`/`homeToday()`/`homeRow()`, `recordProgress()` (progress for a session we are *not* standing in, so it cannot call `dayExercises()`), `recentSessions()`, `lastMissed()`, `accountLine()`.
26. **the calendar** — `renderCalendar()`, `calendarRange()`, `calendarMark()`, `calendarPain()`, `calendarLegend()`.
27. **the programme screen** — `anchorPanel()` (the block anchor, editable in one tap, with the derived weekdays shown so it can be checked rather than trusted), `renderProgrammePage()`.
28. **the account screen** — `settingsGroup()`, `renderAccountPage()`, which labels the two settings scopes explicitly.
29. **exercise head (shared by Overview and Log)** — `exerciseHead(ex)`, `hintChips()`. The rail colour, name and prescription line. **Change an exercise's identity here only**, so the two views cannot show two different prescriptions for one row.
30. **overview card (read-only)** — `overviewCard(ex, s, idx)`, `statusLine()`, `summaryText()`, `painLogged()`. Zero inputs.
31. **the focus logger's instruments** — `parseRestSeconds(ex)`, `restSeconds(s)`, `fmtClock()`, `restClockNode()`, `startRestTick()`; `heaviestSet()`, `agoWords()`, `lastTimeNode()`; `instrumentRail()`. One interval for the whole app, and it only ever repaints.
32. **the set editor (Log view)** — `buildSetEditor()` (recap + chips + count control + fields + actions), `setRecap()`, `collapse()`. An input's `oninput` writes and saves; it must never rebuild a container it lives in.
33. **summary & notes (collapsed, inside Log view)** — `buildSummaryPanel()`, the editable flat fields.
34. **log card** — `logCard(ex, s, idx)` and its `redraw()`.
35. **export** — `exportSets()`, `exportEntry()` (a named whitelist, not a rest-spread — see "State model" below), pure `buildSessionPayload(stored, program)` for export/backfill, `buildSessionExport()`, `exportSession()`, `copyJSON()`.
36. **events** — wiring for markup that lives for the whole page, a guarded `keydown` listener (Escape closes the action sheet, arrows page Log view), service-worker registration, the `boot()` call. Week, day and date are not standing widgets: they are the claim picker and the calendar, built on demand.
...
than always to the hub (one level, because there is nothing deeper to remember). `surface` is derived from `route` and read by the boot/export paths.

**`STATE.date` is primary, and `week`/`day` are derived from it** — read off the claim when a
session is stored on that date, otherwise the schedule's suggestion. The position is never stored separately: a stored
position can disagree with the claim, and when it does it puts the wrong week's prescriptions on
screen. `tp_pos_v1` is retired and survives only in `LEGACY_KEYS`, which cleans it up. The date always starts at **today**, because a screen
...
overwriting. Removing the active programme clears `tp_program_v1` and its active marker,
...
**There is no separate history browser.** Every session on this device is a date on the calendar,
...
- **Opening a date switches sessions, and that is the navigation model.** It is how a session gets corrected or completed after the fact, and now also how the calendar works. (`tp-session-3` moved the morning reading to a pre-session `amPainOnWaking` for this reason; see `docs/data-contracts.md`.)
...
- **The reps field is whatever the exercise measures**, decided by `metricOf(ex)` from the prescription string: `Hold (s)`, `Time (min)`, `Dist (m)`, `Work (cal)`, `Reps`, or `Result` for composite prose. It sets the label, the keyboard (`inputmode`), the placeholder and the unit appended by `deriveReps()`. `inputmode="numeric"` is a digits-only keypad on iOS and makes a 45-second hold untypeable, so the fallback for anything unrecognised is a **full keyboard**, never a numeric one. Composite prose is tested first, or `"45 sec hard / 75 sec easy"` reads as a 45-second hold. The unit lives in the *label*, not as an overlay inside the field: `"min"` plus `"8-10"` does not fit a 68px column and clips the value.
- **Upcoming set chips are inert `<span>`s, and planned-count changes are secondary.** Tapping an upcoming number must do nothing. The quiet **Adjust sets** action opens a bottom sheet whose −/+ controls write `setTarget`, floored at the current set and capped at 12. The primary set path has no steppers. A one-slot exercise renders no chip strip at all.
...
- **The check-in lives near the top of the date view, and never in the focus logger.** It sits above the exercise list and *below* the primary action, so the data is asked for without ever standing between the athlete and a warm-up. Its own status line ("Not filled", or a summary) is how the athlete knows whether it is done. It is still the *same* `sessionCard()`, still autosaves on every input, and still tints itself by readiness — only its host moved. `renderCheckin()` rebuilds *only* that block, because the exercise list below it holds the scroll position a full rebuild would throw away.
- **Committing a set says so, three ways.** A `LOGGED 60×4 · 60×4` recap line above the chips, a toast naming the set, and a one-off pulse on the chip that just landed (plus `navigator.vibrate` where it exists, guarded). All three are needed: the next draft is seeded from the set just logged, so the fields read the same and without them nothing feels recorded. Committing an *empty* draft now toasts why nothing happened instead of being a silent no-op.
...
Inline `<style>` in the head. Colour comes entirely from CSS custom properties (`--bg`, `--panel`, `--ink`, `--accent`, `--good/--warn/--bad`) declared in the four `html[data-theme]` blocks (see section 2b); a 4px spacing scale (`--s1`..`--s5`) and a type scale (`--t-xs`..`--t-hero`). Mobile-first, single column, `max-width: 720px`, `env(safe-area-inset-*)` respected for notch/home-bar.
...
- **One height scale (`--h-field` / `--h-tap` / `--h-primary`) on `:root`.** The primary action must never be shorter than the fields above it.
...
- **The narrow pain column takes `painLblShort()`**, the site alone. Once the label is uppercase and letterspaced, "Knee pain" does not fit ~70px, and the `/10` beside it already says it is a score. Same reasoning as `Dist` over `Distance`. The `aria-label` keeps the full wording.
- **Amber is reserved for *now*** and green for *logged*: the primary action, the rest clock while you are still inside the prescribed rest, the current pip, today's ring on the calendar. Quiet actions (`.quietaction`, `.addnote`) and disclosures are muted and underlined instead — a secondary action that borrows either competes with the one tap that matters.
...
**The Tracked fields section is not multi-user support.** It is account-scoped preferences
following one account — see `docs/design-rationale.md` for why that scope, and the
"Account-scoped settings" section above for the merge. Account identity stays in the auth module
and the owner marker, nothing in the session autosave path becomes keyed by person, and in-app
profile switching remains deferred.

**Nothing renders a tracked field that is switched off** — not the check-in column, the logger's
0–10 row, the calendar's mark or its legend entry, and never a greyed one. **Every surface uses the
athlete's own label** through `painLbl()`, or `painLblShort()` where the column is ~70px. Values
already logged are never deleted by switching a field off, and `tracking` in the export still says
which fields this athlete does not collect, so an empty value stays readable.
