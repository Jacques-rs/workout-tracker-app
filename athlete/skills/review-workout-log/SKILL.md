...
- **`sets[]` wins when non-empty.** It is strictly more information. **Do not average it into one load** — the shape of a session (100 → 80 → 80) *is* the signal, and it is the reason per-set logging exists. Fall back to the flat fields when `sets` is `[]` or absent.
- **The flat `load`/`reps`/`rpe` are the athlete's own summary, not a computed value.** When they disagree with `sets[]`, that is not a bug — it is what the athlete considers the headline number.
